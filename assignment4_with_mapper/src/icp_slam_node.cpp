...
ICPSlamNode::ICPSlamNode() : local_nh_("~")
{
  ...
  local_nh_.param<double>("max_keyframes_time", max_keyframes_time, 15);
 ...
 // cv::Mat(width_, height_, origin_,  CV_8SC1);
...
void ICPSlamNode::laserCallback(const sensor_msgs::LaserScanConstPtr &laser_msg)
{
  ...
  auto is_keyframe = icp_slam_->track(laser_msg, tf_odom_laser, tf_map_laser); //tf_map_laser has the updated pose
  if (is_keyframe)
  {
    //TODO: update the map
...
void ICPSlamNode::publishMap(ros::Time timestamp)
{
  ...
  std::string add="/home/mannatk/sfuhome/maps/"+std::to_string(map_count)+".png";
  //std::string add="/home/mannatk/sfuhome/maps/map.png";

  cv::imwrite(add, map);
  map_count++; //for keeping track of maps
}
